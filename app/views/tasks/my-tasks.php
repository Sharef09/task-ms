...
<div class="d-flex justify-content-between align-items-center mb-2">
    <h5 class="fw-bold mb-0"><i class="fas fa-tasks me-2"></i>My Tasks</h5>
    <?php if (!isStaff()): ?>
...
<form method="GET" action="<?= rtrim($app['url'], '/') ?>/tasks/my-tasks" class="row g-2 mb-4">
    <div class="col-md-3">
    ...
    <div class="col-md-1 d-flex gap-1">
        ...
        <a href="<?= rtrim($app['url'], '/') ?>/tasks/my-tasks" class="btn btn-sm btn-outline-secondary"><i class="fas fa-times"></i></a>
    </div>
</form>
            ...
            <table class="table table-hover align-middle mb-0 small">
                ...
                <tbody>
                    <?php if (!empty($tasks)): ?>
                            ...
                            <td style="min-width:100px;">
                                <div class="d-flex align-items-center gap-2">
                                    <div class="progress flex-grow-1" style="height:6px;">
                                        <div class="progress-bar bg-<?= ($task->progress ?? 0) >= 100 ? 'success' : (($task->progress ?? 0) >= 50 ? 'warning' : 'primary') ?>" role="progressbar" style="width:<?= e($task->progress ?? 0) ?>%"></div>
                                    </div>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="9" class="text-center text-muted py-5"><i class="fas fa-tasks fa-2x mb-2 d-block"></i>No tasks assigned to you</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="card-footer bg-white border-top d-flex justify-content-center py-2">
        <nav><ul class="pagination pagination-sm mb-0">
            ...
            <li class="page-item <?= ($i == ($_GET['page'] ?? 1)) ? 'active' : '' ?>">
                <a class="page-link" href="<?= rtrim($app['url'], '/') ?>/tasks/my-tasks?page=<?= $i ?>"><?= $i ?></a>
            </li>
            <?php endfor; ?>
        </ul></nav>
    </div>
