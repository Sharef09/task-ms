...
<div class="d-flex justify-content-between align-items-center mb-2">
    <h5 class="fw-bold mb-0"><i class="fas fa-exclamation-triangle text-danger me-2"></i>Abused Tasks</h5>
    <?php if (!isStaff()): ?>
...
<form method="GET" action="<?= rtrim($app['url'], '/') ?>/tasks/abused-tasks" class="row g-2 mb-4">
    <div class="col-md-3">
    ...
    <div class="col-12 d-flex gap-1">
        <button type="submit" class="btn btn-sm btn-outline-danger"><i class="fas fa-filter me-1"></i>Filter</button>
        <a href="<?= rtrim($app['url'], '/') ?>/tasks/abused-tasks" class="btn btn-sm btn-outline-secondary"><i class="fas fa-times me-1"></i>Reset</a>
    </div>
</form>
                ...
                <tbody>
                    <?php if (!empty($tasks)): ?>
                        ...
                        <tr class="table-danger">
                            ...
                            <td class="text-nowrap text-danger fw-medium"><?= formatDate($task->due_date ?? null, 'M j, Y') ?></td>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="9" class="text-center text-muted py-5"><i class="fas fa-check-circle text-success fa-2x mb-2 d-block"></i>No abused or overdue tasks</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="card-footer bg-white border-top d-flex justify-content-center py-2">
        <nav><ul class="pagination pagination-sm mb-0">
            ...
            <li class="page-item <?= ($i == ($_GET['page'] ?? 1)) ? 'active' : '' ?>">
                <a class="page-link" href="<?= rtrim($app['url'], '/') ?>/tasks/abused-tasks?page=<?= $i ?>"><?= $i ?></a>
            </li>
            <?php endfor; ?>
        </ul></nav>
    </div>
